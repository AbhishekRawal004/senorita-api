...
class ActionHandler:
            ...
            try:
                with open(USER_DATA_FILE, 'r') as f:
                    ...
                    return {
                        ...
                        **loaded_data,
                        "waiting_for_platform": None,
                        "reminders": loaded_data.get(
                            "reminders",
                            [])}
            except Exception as e:
                print(
                    f"Error loading user data from {USER_DATA_FILE}: {e}. Using default data.")
                return self._get_default_data()
        else:
            print(
                f"User data file {USER_DATA_FILE} not found. Using default data.")
            return self._get_default_data()
    ...
    def _get_weather(self, city: str, speak) -> bool:
        ...
        if "YOUR_OPENWEATHERMAP_KEY_HERE" in WEATHER_API_KEY:
            speak(
                "Weather API Key is placeholder. Please update it in action_handler.py.")
        ...
        try:
            ...
            if data.get("cod") == 200:
                ...
                speak(
                    f"The weather in {city.title()} is currently {weather_desc}, with a temperature of {temp} degrees Celsius.")
                return True
            else:
                error_msg = data.get("message", "unknown error")
                speak(
                    f"Sorry, I couldn't find the weather for {city.title()}. The service reported: {error_msg}.")
                return False
    ...
    def _get_news(self, topic: str, speak) -> bool:
        ...
        params = {
            "q": topic if topic else "technology",
            "apiKey": NEWS_API_KEY,
            "language": "en",
            "pageSize": 3}
        try:
            ...
            if articles:
                ...
                speak(
                    f"Here are the top headlines for '{topic if topic else 'technology'}': 1. {headlines[0]}. 2. {headlines[1]}. 3. {headlines[2]}.")
                return True
            else:
    ...
    def _get_nasa_apod(self, speak) -> bool:
        ...
        try:
            ...
            speak(
                f"Today's NASA picture is titled '{title}'. You can search for it online to see the image.")
            return True
        except requests.exceptions.RequestException as e:
    ...
    def search_for_image(self, query, start_index=1, num_results=10):
        ...
        try:
            ...
            print(
                f"An unexpected error occurred during image search: {e}",
                file=sys.stderr)
    ...
    def _update_conversation_context(
        ...
        if any(
            word in user_input_lower for word in [
                'image',
                'picture',
                'photo',
                'show me']):
    ...
    def handle_image_request(self, user_input, speak):
        ...
        is_follow_up = "more" in user_input_lower and len(
            user_input_lower.split()) <= 3
        query = user_input_lower

        if is_follow_up:
            print(
                f"DEBUG: Follow-up request detected. Last query: {self.last_image_query}, Start index: {self.last_image_start_index}",
                file=sys.stderr)
            if self.last_image_query:
                query = self.last_image_query
                images = self.search_for_image(
                    query, start_index=self.last_image_start_index + 10, num_results=10)
                if images:
        ...
        images = self.search_for_image(
            search_query, start_index=1, num_results=10)

        if images:
            print(
                f"DEBUG: Found {len(images)} images for query: {query}",
                file=sys.stderr)
        ...
        try:
            ...
            if info_type == "favorite":
                ...
                if value and value not in self.user_data["interests"]:
                    ...
                    self._save_data()
                    speak(
                        f"Cool! I've noted that you are interested in **{value}**.")
                    return True

        except Exception as e:
            print(
                f"Error parsing Gemini response for personal info: {e}. Raw: {raw_response}",
                file=sys.stderr)
            pass
    ...
    def _map_location_to_timezone(self, location: str) -> str:
        ...
        if "tokyo" in location or "japan" in location:
            return "Asia/Tokyo"
        ...
        try:
            matches = [
                tz for tz in pytz.all_timezones if location in tz.lower()]
            if matches:
                return matches[0]
        except Exception:
            pass
    ...
    def _call_gemini_api(self, user_query: str, history: list = None):
        ...
        try:
            ...
            system_prompt = f"""
You are Senorita, a helpful, friendly, and intelligent personal assistant.

Current context:
- User name: {self.user_data.get('name', 'friend')}
- Current topic: {self.conversation_context.get('current_topic', 'general')}
- Last action: {self.conversation_context.get('last_action', 'none')}
- User mood: {self.conversation_context.get('user_mood', 'neutral')}
"""
...
"""
    ...
    def _check_for_immediate_platform_and_launch(self, query: str, speak):
        ...
        if platform_name:
            ...
            if not media_query or media_query in [
                    "play", "search", "watch", "listen"]:
                clean_query_match = re.search(
                    r'(play|listen to|watch|search for)\s+(.*?)', query_lower)
    ...
    def _extract_time_and_task(self, query: str):
        ...
        try:
            ...
            print(
                f"Error parsing Gemini response for reminder extraction: {e}",
                file=sys.stderr)

        return {
            "task": query,
            "task_only": query.strip(),
            "reminder_time": None}
    ...
    def handle_recall_notes(self, speak):
        ...
        if not reminders:
            ...
            for i, r in enumerate(reminders):
                ...
                time_suffix = f" (Remind: **{remind_at}**)" if remind_at and remind_at.lower(
                ) != 'null' else ""

                reminder_text_display.append(
                    f"- **{i+1}.** {note}{time_suffix} (Added: {added_date})")

                speak_time = f"for {remind_at.replace('AM', ' A. M.').replace('PM', ' P. M.')}" if remind_at and remind_at.lower(
                ) != 'null' else ""
                reminder_text_speak.append(
                    f"Number {i+1}, {note} {speak_time}")

            reminder_text_display = "\n".join(reminder_text_display)
            reminder_text_speak = ", ".join(reminder_text_speak)

            speak(
                f"You currently have {len(reminders)} items on your list. They are: {reminder_text_speak}.")
    ...
    def handle_mobile_control(self, intent: str, slots: dict, speak):
        ...
        return {
            "type": "text",
            "content": "Mobile control command recognized but not processed."}
        ...
        try:
            if local_intent in [
                ...
                if tz_name:
                    ...
                    speak(
                        f"I don't recognize the location '{location_raw}'. I can search for it instead.")
                    current_query = f"what is the current time in {location_raw}"
                    answer = self._call_gemini_api(
                        current_query, self.conversation_history)
                ...
                # Clear conversation history to prevent Gemini from overriding
                # our response
                self.conversation_history = []
                ...
                return self.handle_send_message(
                    message, contact, speak, platform=platform)
                ...
                if fallback_query:
                    ...
                    if self._extract_and_save_personal_info(
                            fallback_query, speak):
                        self.conversation_history = []
                        answer = "Personal info saved."
                    else:
                        speak(
                            "Hmm, I'm not sure what you mean, but let me try searching for it anyway.")
                        answer = self._call_gemini_api(
                            fallback_query, self.conversation_history)
                else:
            ...
            if answer:
                if local_intent in [
                        ...
                        "search"] or answer in [
                        "Personal info saved.",
                        "I need a query to search for."]:
                    speak(answer)

                if current_query:
                    self.conversation_history.append(
                        {"role": "user", "parts": [{"text": current_query}]})
                    self.conversation_history.append(
            ...
            print(
                f"Action Execution Error for '{local_intent}': {e}",
                file=sys.stderr)
            speak(error_msg)
